...
namespace pcl {
	namespace registration
	{
		...
		public:
			...
			typedef std::shared_ptr<GRORInitialAlignment<PointSource, PointTarget, Scalar> > Ptr;
			typedef std::shared_ptr<const GRORInitialAlignment<PointSource, PointTarget, Scalar> > ConstPtr;
			...
			GRORInitialAlignment() : best_count_(3)
			{
				...
				//input_ = PointCloudSource().makeShared();
				//target_ = PointCloudSource().makeShared();
		...
		protected:
			...
			*/
			...
			/* \brief params used to calculate MSAC*/
			float delta_;
			/* \brief MSAC socre*/
		...
		private:
			...
			float
				calculateMSAC(Eigen::Matrix4f trans);
